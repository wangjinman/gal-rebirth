...
> **最后更新**：2026-05-29（黑屏审计补充 · LWT_07～10 / BG_29～34 / CG_04～07）  
...
### 2026-05-28 ~ 2026-05-29 · 黑屏审计补充素材（林晚棠线）

> 来源：`美术素材\第一批素材_AI绘图提示词.md` v1.9 黑屏审计清单。  
> 用户手动复制到 `开发\RenPy项目\GALCS\`；AI **默认只写** `美术资源初稿\`。

#### 立绘 · LWT_07～10（括号表情清理配套）

| 文件 | 表情 | 备注 |
|------|------|------|
| `LWT_07_angry.png` | 生气 | 用户保留鲜艳原色，**撤销 despill** |
| `LWT_08_gentle.png` | 温柔 | 母版 + bust |
| `LWT_09_sad.png` | 悲伤站立 | |
| `LWT_09_sad_action.png` | 悲伤捂胸 | 动作差分 |
| `LWT_10_thinking.png` | 思考 | 含思考姿势 |

流程：洋红 `#FF00FF` → `立绘\scripts\sprite_chroma_feather.py`（色键 + 羽化 + 自动 bust）。

#### 背景 · BG_29～34

| ID | 定稿文件 | 状态 | 备注 |
|----|----------|------|------|
| BG-29 | `BG_29_restroom_mirror_day.png` | ✅ | Day1 洗手间镜前 |
| BG-30 | `BG_30_playground_bleachers_noon.png` + `_empty.png` | ✅ | 操场台阶午休；empty 供立绘叠层 |
| BG-31 | `BG_31_rooftop_afternoon_day.png` | ⚠️ 待补 | 与 BG-04 同机位日景；对话摘要曾交付，J 盘文件缺失 |
| BG-32 | `BG_32_living_room_warm_evening.png` | ⚠️ 待补 | TE 客厅，比 BG-11 更暖 |
| BG-33 | `BG_33_university_gate_september_morning.png` + `_empty.png` | ✅ | TE 大学校门开学日 |
| BG-34 | `BG_34_apartment_entrance_rainy_night.png` | ⚠️ 待补 | 2008 住宅楼雨夜；**无文字、无伞架**；`v1_text` 为含字备份 |

#### 事件 CG · CG_04～07

| ID | 定稿文件 | 剧本节点 | 备注 |
|----|----------|----------|------|
| CG-04 | `CG_04_be_empty_seat.png` | BE 空座位 | `02_lindao_route.rpy` BE ~7492 |
| CG-05 | `CG_05_day16_freezing_point.png` | Day16 冰点 | 黑板 **粉笔手写**「距离高考 15天」；勿脚本叠字 |
| CG-06 | `CG_06_day18_note_park.png` | Day18 转机纸条 | 与 BG-07 同系列公园；中景递纸条 |
| CG-07 | `CG_07_te_reunion_sakura.png` | TE 樱花重逢 | 情侣牵手朝镜头；`show cg te_reunion with fade; pause 4.0` |

**CG-05 修订履历**：v1 120天原图 → 脚本叠字失败（`v1_bad_edit`）→ AI 重生成 15 天粉笔版定稿。  
**CG 备份约定**：每张保留 `*_original.png`（通常 1536×1024）；正式版居中裁切 **1920×1080**。

#### Ren'Py 引用速查

```renpy
# CG
image cg be_empty_seat = "CG/CG_04_be_empty_seat.png"
image cg day16_freezing_point = "CG/CG_05_day16_freezing_point.png"
image cg day18_note_park = "CG/CG_06_day18_note_park.png"
image cg te_reunion = "CG/CG_07_te_reunion_sakura.png"
```
...
12. **CG 黑板/招牌文字**：禁止 PIL/脚本叠字（易出数字字体、边缘瑕疵）；用 AI 重生成并在提示词写明粉笔手写样式。
13. **立绘 despill**：LWT_07 angry 用户偏好鲜艳原图，勿擅自 despill。
14. **交付核验**：复制到 J 盘后确认 **定稿 + `_original` 备份** 均存在；仅留 original 时可用 Python 居中裁切补正式版。
15. **GenerateImage**：偶发 permission denied → 先写入 Cursor `assets\`，再 `shutil.copy2` 到 J 盘。
16. **CG 人物细节**：CG-06 纸条呈小信封形态可接受；领结格纹与立绘菱形纹略有差异，修订须用户点名。
...
| 2026-05-28 | 提示词 v1.9：黑屏审计新增 BG_29～34、CG_04～07、LWT_07～10 |
| 2026-05-28 | LWT_07～10 立绘交付（angry/gentle/sad/sad_action/thinking） |
| 2026-05-29 | BG_29 洗手间镜前、BG_30 操场台阶（含 empty） |
| 2026-05-29 | BG_33 大学校门开学日（含 empty） |
| 2026-05-29 | CG_04 BE 空座位、CG_05 Day16 冰点（15天粉笔版定稿） |
| 2026-05-29 | CG_06 Day18 公园递纸条、CG_07 TE 樱花重逢 |
| 2026-05-29 | CG_04/06/07 正式版从 `_original` 补裁 1920×1080 |
| 2026-05-29 | 第十节合并黑屏审计批次履历；经验备忘增 CG 叠字/交付核验 |
